...
//Forward
mat3 cotangent_frame(in vec3 nor, in vec3 pos, in vec2 uv);

void main()
{
	mat3 TBN = cotangent_frame(vNormal, vPosition, vUV);
	vec3 N = TBN * (texture(normalMap, vUV).xyz * 2 - 1); //Affects how bright the model is
...
mat3 cotangent_frame( in vec3 nor, in vec3 pos, in vec2 uv )
{
    ...
    vec3 dp2perp = cross( dp2, nor ); // determine right
    vec3 dp1perp = cross( nor, dp1 ); // determine up
